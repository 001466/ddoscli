...
g++ -olan.exe lan.cpp -Wl,-Bstatic -lwsock32  -liphlpapi -lpthreadgc2 -Wl,-Bdynamic -lwpcap
...
void *fnSendPacket(void *arg){
	...
	int size = fnEncodePacket( packet, pcDstIpAddr,0, pcDstIpAddr,*pnDstPort,pDev->szDevMac,pDev->szDevGatewayMac);
	...
	*(unsigned short *)(pPsdHeader->tcpLen)  = htons(sizeof(TCP_HEADER));	
	...
	unsigned int   unSrcIpAddr = inet_addr(pcDstIpAddr);
